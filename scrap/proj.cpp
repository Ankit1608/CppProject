...
void run_pipe(char *commands[], int n_commands) {
    ...
    for (int i = 0; i < n_commands; i++, in = fd[0], close(fd[1])) {
        ...
        char *args[MAXIMUM_INSTRUCTIONS], *token, *input_file = nullptr, *output_file = nullptr;
        for (token = strtok(commands[i], " "); token != nullptr; token = strtok(NULL, " ")) {
            if (strcmp(token, "<") == 0) {
                redirect_in = 1;
...
while (!done) {
    std::cout << "$ ";
    std::cin.getline(input, MAX_INS_LENGTH);
